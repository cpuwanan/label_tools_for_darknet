...
public:
	MyImage();
	MyImage(cv::Mat image, std::string id, int index);
  ~MyImage();
  void init(cv::Mat image, std::string id, int index);
  bool addObjectsFromLabelFile(std::string file, std::map<std::string, int> annotations);
